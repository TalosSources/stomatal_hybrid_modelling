...
def train_general(
    ...
    """
    train_general is a general torch training method.
    Args:
    ...
    for i in range(epochs):
        ...
        if (
            torch.isnan(loss) or torch.isinf(loss) or (output == 0.0).any()
        ):  # Safety measure to avoid breaking training, shouldn't happen in practice
            if printIter:
...
def perform_hp_tuning(config, data):
    ...
    data, _ = train_test_split(
        data, test_size=config.train.test_split, random_state=config.train.seed
    )

    model_func = models.rs_model if config.pipeline.predict_rs else models.gsCO2_model
    sub_model_producer = lambda hps: model_func(
        hidden_size=hps["hidden_size"],
        n_hidden=hps["n_hidden"],
        activation=config.model.activation,
        batch_norm=hps["batch_norm"],
    )
    ...
    # choose an optimizer
    opt_producer = lambda parameters, hps: torch.optim.Adam(
        parameters, lr=hps["lr"], weight_decay=hps["weight_decay"]
    )

    # choose a data iterator
    iterator_producer = lambda data: batch_ctx_dict_iterator(
    ...
    for hp_set in grid_search_hp_iterator:
        total_score, loss, benchmark, coeff_determination = k_folds(
            config.train,
            np.array(data),
            model_producer,
            opt_producer,
    ...
    print(
        f"Finished hp-tuning: found best hp-set = {best_hps}, yielding score {lowest_score}"
    )
...
def train_and_evaluate_pipeline(config, data):
    config.seed = config.get("seed", 24)  # NOTE: Might be moved outside this function
    ...
    model = (
        models.rs_model_from_config(config.model)
        if config.pipeline.predict_rs
        else models.gsCO2_model_from_config(config.model)
    )
    ...
    data_iterator = batch_ctx_dict_iterator(
        train_data, batch_size=config.train.batch_size
    )
    ...
    eval_after_training_train = eval.eval_general(
        model_wrapper, train_data, loss_criterion
    )
    print(
        f"Eval after training on train data (to check overfitting): {eval_after_training_train}"
    )

    # eval the empirical model (for comparaison)
    empirical_model_wrapper = pipelines.make_pipeline(None, None, None, output_rs=False)
    eval_empirical_model = eval.eval_general(
        empirical_model_wrapper, test_data, loss_criterion
    )
    ...
    empirical_coeff = eval.coefficient_of_determination(
        empirical_model_wrapper, test_data
    )
    ...
    if config.get("graphic", False):
        plot.plot_losses(
            [losses],
            labels=["Loss"],
            coefficients=[model_coeff],
            minmax=False,
            smoothing=0.1,
        )
...
def k_folds(
    config, data, model_producer, opt_producer, criterion, iterator_producer, hp_set
):
    k = config.k_folds
    ...
    for train_ids, test_ids in kfold.split(data):
        ...
        with torch.no_grad():
            ...
            coeff_determination += eval.coefficient_of_determination(
                model, test_data_iterator
            )
...
class batch_ctx_dict_iterator:
    def __init__(self, data, batch_size=16):
        ...
        grouped_data, probs = utils.group_by_ctx_id(
            data
        )  # data is a dict ctx_id -> array of data points with the same context set
...
rs_dummy_data_generator = function_sample_iterator(
    f=lambda x: empirical_model(x), sampler=rs_sampler()
)
