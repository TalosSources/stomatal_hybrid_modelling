...
def load_pipeline_data_dict_from_all_sites(
    config,
    predictor_keys=None,
    constant_keys=None,
    output_keys=None,
    ...
    for site_name in config.sites:
        ...
        load_pipeline_data_dict_single_site(
            ...
            verbose=verbose,
            exclude_negative_outputs=config.exclude_negative,
        )
...
def load_pipeline_data_dict_single_site(
    predictor_path,
    observation_path,
    data,
    nPoints=None,
    ...
    predictor_arrays = {
        ctx: {
            k: torch.tensor(
                (
                    predictor_data[k_m][:, 0:1]
                    if predictor_data[k_m].shape[1] == 2
                    else predictor_data[k_m]
                ).astype(np.float32)
            ).flatten()[:nPoints]
    ...
    # Load the precipitation array: only used to filter out timesteps
    precipitation_array = observation_data["Pr"].astype(np.float32).flatten()[:nPoints]
    ...
    def is_valid_timestep(i):
        return (
            ...
            )  # Filter out nan Q_LE values!
            and (
                (not exclude_negative_outputs)
                or (not (output_arrays[output_keys[0]][i] < 0.0))
            )  # Filter out negative Q_LE values! actually should we? our pipeline can actually output negative values
            # Filter out timesteps with precipitation (unreliable flux)
            # NOTE: For some regions, this might filter out a lot of timesteps... NOTE: Should we remove timesteps > 0? or some tolerance threshold?
            # and not precipitation_array[i] > 0.0
    ...
    def is_valid_context(ctx, i):
        return (
            predictor_arrays[ctx]["ra"][i]
            != 0.0  # ra is in the denom of the PM equation: can't be 0.
            and not predictor_arrays[ctx]["IPAR"][
                i
            ].isnan()  # IPAR is used in the pb pipeline.
            ...
            and not constants[ctx][
                "CT"
            ].isnan()  # AU-TTE has nan values for L constants
        )
            ...
            data.append(
                (
                    (
                        {
                            ctx: {
                                k: predictor_arrays[ctx][k][i].unsqueeze(0)
                                for k in predictor_arrays[ctx]
                            }
                            | constants[ctx]
                            | {"DS_": 0.5}
