...
* variations of the normal pipeline, including the function mapping the output of the FCN to a positive number
* the inv pipeline, a FCN computes rs, trained to match the inverted PM rs_hat reconstruction from Q_LE
TO CONFIG: which pipeline to use?
"""
import inspect
import torch
...
def make_pipeline(gsCO2_model, Vmax_model, output_rs=False):
    ...
    def subpipeline(predictors):
        ...
        rs = pb(**pb_predictors, rs_model=gsCO2_model, Vmax_model=Vmax_model) # TODO: rename gsCO2 model name?
