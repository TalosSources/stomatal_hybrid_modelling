...
dth = dt/3600; %% [hour]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
V=zeros(NN,ms); % Volume of liquid water stored in the soil layer
O=zeros(NN,ms); %Soil Moisture – Liquid volumetric Soil Water Content 
...
%Sdp=zeros(NN,ms);
Vice=zeros(NN,ms); % Volume of frozen water stored in the soil layer
Oice=zeros(NN,ms); %Frozen volumetric Water content
%%%%%%%%%%%%%%%%%%%%%%%%%%%
OF=zeros(NN,1); OS=zeros(NN,1); % Soil Moisture First Soil Layer 
% NN = 100; % Option to do few iterations for fast debugging
...
ran_L_final=zeros(NN,cc);
ran_H_final=zeros(NN,cc);
PAR_sun_L_final=zeros(NN,cc);
PAR_shd_L_final=zeros(NN,cc);
PAR_sun_H_final=zeros(NN,cc);
PAR_shd_H_final=zeros(NN,cc);
Vmax_sun_L_final=zeros(NN,cc);
Vmax_shd_L_final=zeros(NN,cc);
Vmax_sun_H_final=zeros(NN,cc);
Vmax_shd_H_final=zeros(NN,cc);

Psi_sun_L_final=zeros(NN,cc);
Psi_shd_L_final=zeros(NN,cc);
Psi_sun_H_final=zeros(NN,cc);
Psi_shd_H_final=zeros(NN,cc);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
OPT_FR_SOIL = 1; 
%%%%
if  not(exist('OPT_VCA','var')) % Option for computing forest structural attributes including a variable Crown Area
    OPT_VCA = 0;
    OPT_ALLOME = 0;
end
...
for i=2:NN
    ...
    [V(i,:),Vice(i,:),O(i,:),Oice(i,:),ZWT(i),OF(i),OS(i),OH(i,:),OL(i,:),Psi_s_H(i,:),Psi_s_L(i,:),Rd(i),Qi_out(i,:),WTR(i,:),...
        ...
        ran_L_final(i,:),ran_H_final(i,:),...
        PAR_sun_L_final(i,:),PAR_shd_L_final(i,:),PAR_sun_H_final(i,:),PAR_shd_H_final(i,:),...
        Vmax_sun_L_final(i,:),Vmax_shd_L_final(i,:),Vmax_sun_H_final(i,:),Vmax_shd_H_final(i,:),...
        Psi_sun_L_final(i,:),Psi_shd_L_final(i,:),Psi_sun_H_final(i,:),Psi_shd_H_final(i,:)]=HYDROLOGIC_UNIT(V(i-1,:),Oice(i-1,:),aR,Zs,...
