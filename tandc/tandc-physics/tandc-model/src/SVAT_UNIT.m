...
function[Ts,Pr_sno,Pr_liq,Ws_under,Csno,Cice,Cfol_H,Cfol_L,CLitter,NDVI,rb_H,rb_L,rs_sunH,rs_sunL,rs_shdH,rs_shdL,r_litter,...
    ...
    dQVEG,HV,QEV,TsV,WAT,q_run,EG_dis,J_Hdis,J_Ldis,Imelt,Smelt,...
    ran_L,ran_H,...
    PAR_sun_L_svat,PAR_shd_L_svat,PAR_sun_H_svat,PAR_shd_H_svat,...
    Vmax_sun_L_svat,Vmax_shd_L_svat,Vmax_sun_H_svat,Vmax_shd_H_svat,...
    Psi_sun_L_svat,Psi_shd_L_svat,Psi_sun_H_svat,Psi_shd_H_svat]=SVAT_UNIT(Tstm0,Pr,Ta,Ds,Ws,zatm,Tstm1,dt,ea,Latm,N,Pre,...
...
ran_L=zeros(1,length(Ccrown));
ran_H=zeros(1,length(Ccrown));

PAR_sun_L=zeros(1,length(Ccrown));
PAR_shd_L=zeros(1,length(Ccrown));
PAR_sun_H=zeros(1,length(Ccrown));
PAR_shd_H=zeros(1,length(Ccrown));

PAR_sun_L_svat=zeros(1,length(Ccrown));
PAR_shd_L_svat=zeros(1,length(Ccrown));
PAR_sun_H_svat=zeros(1,length(Ccrown));
PAR_shd_H_svat=zeros(1,length(Ccrown));

Vmax_sun_L_svat=zeros(1,length(Ccrown));
Vmax_shd_L_svat=zeros(1,length(Ccrown));
Vmax_sun_H_svat=zeros(1,length(Ccrown));
Vmax_shd_H_svat=zeros(1,length(Ccrown));

Psi_sun_L_svat=zeros(1,length(Ccrown));
Psi_shd_L_svat=zeros(1,length(Ccrown));
Psi_sun_H_svat=zeros(1,length(Ccrown));
Psi_shd_H_svat=zeros(1,length(Ccrown));
...
for i=1:length(Ccrown)
    ...
    if ((LAI_H(i) > 0) && (OPT_VegSnow==0)) || ((LAI_H(i)>0) && (OPT_VegSnow==1) && (dw_SNO <= 0.5))
        ...
        [rs_sunH(i),rs_shdH(i),Ci_sunH(i),Ci_shdH(i),An_H(i),Rdark_H(i),Lpho_H(i),SIF_H(i),~,...
            PAR_sun_H_svat(i),PAR_shd_H_svat(i),Vmax_sun_H_svat(i),Vmax_shd_H_svat(i),...
            Psi_sun_H_svat(i),Psi_shd_H_svat(i)]=Canopy_Resistence_An_Evolution(PAR_sun_H(i),PAR_shd_H(i),LAI_H(i),...
            ...
            CT_H(i),Vmax_H(i),DSE_H(i),Ha_H(i),FI_H(i),Oa,Do_H(i),a1_H(i),go_H(i),e_rel_H(i),e_relN_H(i),gmes_H(i),rjv_H(i),mSl_H(i),Sl_H(i),Opt_CR,"sunH");
        %disp(PAR_sun_H_svat(i))
    else
    ...
    if (LAI_L(i) > 0) && (Csno == 0) && (Cice == 0)
        ...
        ran_L(i) = ran;
        %%%
        [rs_sunL(i),rs_shdL(i),Ci_sunL(i),Ci_shdL(i),An_L(i),Rdark_L(i),Lpho_L(i),SIF_L(i),~,...
            PAR_sun_L_svat(i),PAR_shd_L_svat(i),Vmax_sun_L_svat(i),Vmax_shd_L_svat(i),...
            Psi_sun_L_svat(i),Psi_shd_L_svat(i)]=Canopy_Resistence_An_Evolution(PAR_sun_L(i),PAR_shd_L(i),LAI_L(i),...
            ...
            CT_L(i),Vmax_L(i),DSE_L(i),Ha_L(i),FI_L(i),Oa,Do_L(i),a1_L(i),go_L(i),e_rel_L(i),e_relN_L(i),gmes_L(i),rjv_L(i),mSl_L(i),Sl_L(i),Opt_CR,"sunL");
    else
