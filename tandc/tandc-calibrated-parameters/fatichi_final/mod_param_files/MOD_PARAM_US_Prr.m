...
%Evergreen Needleleaf Forests: Lands dominated by woody vegetation with a percent cover >60% and height exceeding 2 meter

%The dominant overstory tree species in this forest is black spruce (Picea mariana),
%with the forest floor nearly covered by a moss layer of rusty peat moss (Sphagnum fuscum) and splendid
%feather moss (Hylocomium splendens), and partly covered by tussock with cotton-grass (Eriophorum vaginatum). The understory is dominated by low
%shrubs and herbs such as Labrador tea (Ledum groenlandicum), bog bilberry (Vaccinium uliginosum), dwarf
%birch (Betula nana), and cloudberry (Rubus chamaemorus). This lowland black spruce forest is considered a bog fores
	
%This site is located in a blackspruce forest 
%LAI = 0.73 
%A 17-m tower
%% GPP 650-700 

%The ground surface was covered with a thick moss layer.  the layer of 0e14 cm surface depth
%was the moss layer. The layer of 14e25 cm surface depth was an
%undecomposed organic layer, and at 25-39 cm was a decomposed organic layer
%
 %active layer depth was at 43 cm
...
zatm = 17; %% Reference Height
%%%%%%%%%%%%%%%%%%
%%%% LAND COVER PARTITION
Cwat = 0; Curb = 0 ; Crock = 0;
Cbare = 0.4; Ccrown = [0.6];
%%%%%%%%%% SOIL INPUT - clay/silt
Pcla= 0.10; 
Psan= 0.25;
Porg= 0.19;
Color_Class = 0;
...
Oice = 0;
%%%%%%%%%%%%%
Zs= [0 10 20 50 100 150 200 300 400 500 600 700 800 1000 1500 2000 2500 3000 3500 4000]; %% ms+1
Zdes = 10;
Zinf=  10;
Zbio = 250; 
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.017]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [62]; %[kgC/kgN ] Leaf Nitrogen Concentration
...
Tcold_H = [-50]; %% [°C] Cold Leaf Shed
drn_H=  [1/800]; %% turnover root  [1/d]
dsn_H= [1/600]; % normal transfer rate sapwood [1/d]
age_cr_H= [1220]; %% [day] Critical Leaf Age
...
LAI_min_H = [0.001];
Trr_H = [0.5]; %% Translocation rate [gC /m^2 d]
mjDay_H = [220]; %% Maximum Julian day for leaf onset
LDay_min_H =[14.1]; %% Minimum Day duration for leaf onset
LtR_H = [0.8]; %%% Leaf to Root ratio maximum 
Mf_H= [1/50]; %% fruit maturation turnover [1/d]
Wm_H= [0] ; % wood turnover coefficient [1/d]
eps_ac_H = [0.8]; %% Allocation to reserve parameter [0-1] 
LDay_cr_H = [11.8]; %%%  Threshold for senescence day light [
Klf_H =[1/30]; %% Dead Leaves fall turnover [1/d]
...
LDay_cr_L = [11.5]; %%%  Threshold for senescence day light [h]
...
Ts(1)=Ta(1)+2;
Tdamp(1)=Ta(1); 
Tdp(1,:)= [   -7.7757   -7.8241   -7.8686   -7.9184   -7.9590   -7.9799   -7.9791   -7.8686   -7.6642   -7.3673   -6.9938,...
   -6.6480   -6.1951   -4.9798   -3.4213   -2.7830   -2.8145   -3.0049   -3.1335]; 
TdpI_H(1,:)=-6;
TdpI_L(1,:)=-6;
...
Ws_under(1)=1; 
%%%%%%%%%%%%%% Volume [mm]
O(1,:)=   [  0.1198    0.1196    0.1195    0.1193    0.1191    0.1190    0.1190    0.1195    0.1203    0.1215    0.1232,...
    0.1249    0.1272    0.1348    0.1488    0.1571    0.1566    0.1539    0.1522];
%Oice(1,:)=   [      0.0942    0.2218    0.9937    0.9980    1.0044    0.8461    0.3396    0.3982    0.2880    0.2448    0.1734,...
%    0.4508    0.4718    0.2379    0.1204    0.1124    0.1112    0.1106    0.1070];
%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
Vice(1,:) = (Oice(1,:)).*dz;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
