...
%These coarse alluvial deposits are covered with a few tens centimeters of organic material and fine-grained eolian deposits (loess), which typically stem from wind erosion in the
%braided riverbed when it dries out in autumn. The site.s soil organic carbon content in
%the uppermost 100 cm of soil is about 30 kgC m-2. 
%The vegetation at the Adventdalen site features Salix polaris in drier areas and Eriophorum scheuchzeri and Carex subspathacea in wetter locations. 
%The moss cover is sparse in drier polygons where shrubs dominate the vegetation community, while the wetter
%areas at local depressions feature an almost continuous moss cover

%measurement height was 2.8 m above ground level.
...
%%%%%%%%%%%%%%
%%%%%%%%%%%%%
Zs = [ 0 10 20 50 100 150 200 250 300 400 500 600 700 800 1000 1250 1500 2000 2500 3000 4000 5000 ]; %%% [ms+1]
Zdes = 10;
Zinf=  10;
Zbio = 250;
...
Th_Pr_sno = 3.0; %%% [mm/day] Threshold Intensity of snow to consider a New SnowFall
%%%% ICE Parameter
%%%% ICE Parameter
...
Ha_L =[62]; %% [kJ / mol K]  entropy factor - Plant Dependent
gmes_L=[Inf];
rjv_L= [1.8];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% Matric Potential
...
%%% Leaf
PsiL00_H =  -0.9 ;%%[MPa]  Water Potential at 50% loss conductivity
PsiL50_H =  -4.0; %% [MPa]  Water Potential at 2% loss conductivity
...
[PFT_opt_L(1)]=Veg_Optical_Parameter(12);
OM_H=1;
OM_L=1;
%%%%
...
dd_max_H= [1/45]; %%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  2/365; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_H = [7]; %% [°C] Cold Leaf Shed
drn_H=  [1/795]; %% turnover root  [1/d]
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [0.014]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
r_L = [0.022];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
...
Bfac_lo_L= [0.50]; %% Leaf Onset Water Stress
...
LDay_min_L =[19.0]; %% Minimum Day duration for leaf onset
...
LDay_cr_L = [19.0]; %%%  Threshold for senescence day light [h]
...
Ts(1)=Ta(1)+2;
Tdamp(1)=0;
Tdp(1,:)= [   -7.3293   -7.3373   -7.3506   -7.3646   -7.3635   -7.3431   -7.3038   -7.2453   -7.1264   -6.8697   -6.5127   -6.0989,...
   -5.6381   -5.0402   -4.3216   -3.6496   -2.8650   -2.3470   -2.1049   -1.9517   -1.9577];
TdpI_H(1,:)=-5;
TdpI_L(1,:)=-5;
...
O(1,:)=  [  0.1335    0.1335    0.1334    0.1334    0.1334    0.1335    0.1336    0.1338    0.1342    0.1351    0.1365    0.1382,...
    0.1402    0.1432    0.1474    0.1522    0.1593    0.1653    0.1688    0.1712    0.1711];
Oice(1,:)=   [  0.3016    0.1457    0.4028    0.3606    0.3513    0.4028    0.4027    0.4025    0.3512    0.1943    0.1589    0.1486,...
    0.0951    0.3706    0.3888    0.3841    0.3770    0.3709    0.3675    0.3651    0.3652];
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%Vmin= 0*dz;
%Vmax= (Osat-Ohy)*dz;
