...
Oice = 0;
%%%%%%%%%%%%%
Zs= [0 10 20 50 100 150 200 300 400 500 600 700 800 1000]; %% ms+1
Zdes = 10;
Zinf=  10;
Zbio = 250;
...
Th_Pr_sno = 3; %%% [mm/day] Threshold Intensity of snow to consider a New SnowFall
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% ICE Parameter
...
%%------------------------
%%% Stomata %% -1.4 4.0
Psi_sto_00_L = -1.0;%  %% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_L = -3.0;%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_L =  -2.5; %% [MPa]  Water Potential at PLCs% loss conductivity
PsiL50_L =  -4.5 ;%%[MPa]  Water Potential at 50% loss conductivity
...
[PFT_opt_L(1)]=Veg_Optical_Parameter(11);
OM_H=1;
OM_L=1;
%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [ NaN]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [NaN]; %[kgC/kgN ] Leaf Nitrogen Concentration
[Stoich_H(1)]=Veg_Stoichiometric_Parameter(Nl_H(1));
r_H =  [ NaN];  %% [0.066 -0.011] respiration rate at 10° [gC/gN d ]
gR_H=  [ NaN];% [0.22 - 0.28] growth respiration  [] -- [Rg/(GPP-Rm)]
%LAI_max_H=  [ NaN];
aSE_H=  [ NaN ]; %% Allocation to reserve carbohydrate -- 1 Seasonal Plant --  0 Evergreen
dd_max_H=  [ NaN];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  [ NaN ]; %% [Factor of increasing mortality for cold]
Tcold_H =  [ NaN]; %% [°C] Cold Leaf Shed
drn_H=   [ NaN ]; %% senescence rate leaf  [1/d]
dsn_H=  [ NaN ]; % [ normal transfer rate sapwood [1/d] ]
age_cr_H= [ NaN ]; %% [day] Critical Leaf Age
Bfac_lo_H=  [ NaN ]; %% PHENOLOGY Leaf Onset Water Stress [0-1]
Bfac_ls_H=  [ NaN ]; %% PHENOLOGY Leaf Shed Water Stress [0-1]
Tlo_H =  [ NaN ]; %% Mean Temperature for Leaf onset
Tls_H =  [ NaN ]; %% Mean Temperature for Leaf Shed
PAR_th_H= [NaN]; 
dmg_H=  [ NaN ]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_H =  [NaN];
Trr_H =  [ NaN ]; %% Translocation rate [gC /m^2 d]
mjDay_H = [ NaN ]; %% Maximum Julian day for leaf onset
LDay_min_H =[NaN]; %% Minimum Day duration for leaf onset
LtR_H = [NaN ]; %%% Leaf to Root ratio maximum
Mf_H= [1/50]; %% fruit maturation turnover [1/d]
Wm_H= [0] ; % wood turnover coefficient [1/d]
eps_ac_H = [NaN]; %% Allocation to reserve parameter [0-1]
LDay_cr_H = [ NaN]; %%%  Threshold for senescence day light [h]
...
Nl_L= [35]; %[gC/gN ] Leaf Carbon-Nitrogen ratio
...
%LAI_max_L= [2.0 10.0];
aSE_L= [1]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops
dd_max_L= [1/100];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_L =  [30/365]; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_L = [1.0]; %% [°C] Cold Leaf SLed
drn_L=  [1/1600]; %% turnover root  [1/d]
dsn_L= [ 1/1800]; % normal transfer rate sapwood [1/d]
...
LtR_L = [1.0]; %%% Leaf to Root ratio maximum
Mf_L= [1/50]; %% fruit maturation turnover [1/d]
Wm_L= [0] ; % wood turnover coefficient [1/d]
eps_ac_L = [1.0]; %% Allocation to reserve parameter [0-1]
LDay_cr_L = [12.0]; %%%  Threshold for senescence day light [h
Klf_L =[1/50]; %% Dead Leaves fall turnover [1/d]
fab_L = 0.75; %% fraction above-ground sapwood and reserve
fbe_L = 0.25; %% fraction below-ground sapwood and reserve
ff_r_L= [0.1];
...
D(1)=0.3;
Ts(1)=10;
Tdp(1,:)=[  -1.8034   -1.6233   -1.3317   -0.8977   -0.5279   -0.2240    0.0480    1.9746    3.3981    4.5085    5.3615    5.9827    6.5876];
ros(1)= 100;
Tdamp(1)=Ts(1);
TdpI_H(1,:)=3;
TdpI_L(1,:)=3;
...
Ws_under(1)=1;
%%%%%%%%%%%%%% Volume [mm]
O(1,:)= [0.1602    0.1639    0.1711    0.1864    0.2091    0.2529    0.2745    0.2872    0.2955    0.3014    0.3056    0.3086    0.3114];
%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
%%%%%%%%%%%%%%%%%
cd(cur_dir)
