...
%%% upland mature deciduous forests north wisconsin 

%Hc =24 LAI 4.2 NPP 511 
%Fine Root 180 Leaf 135 Wood 185 
%Sugar maple, basswood, and green ash
% sandy loams or loamy sands (mean texture in top 20 cm: 63 % sand, 31 % silt, 6 % clay, 5–12 % rock fragments)
%  LAI is 5.3 GPP 1100 

%LAI 5.4
%GPP around 1200 see Heinsch et al 2006

%annual precipitation and temperature: 818mm and 4.8°C (1961-1990) http://cheas.psu.edu/fieldsites/wcreek/wcreek.html
% our data: 702mm and 5.4°C 1999-2014
% sugar maple (Acer saccharum Marsh.) 68%, American basswood (Tilia americana L.) 24%, 
% green ash (Fraxinus pennsylvanica Marsh.) 8%; Understory: overstory saplings, 
% eastern leatherwood (Dirca palustris L.), northern maidenhair (Adiantum pedatum L.), 
...
Oice = 0;
%%%%%%%%%%%%%
Zs= [0 10 20 50 100 200 300 400 600 800 1000 1200 1400]; %% ms+1
Zdes = 10;
Zinf=  10;
Zbio = 250;
...
rjv_H=[2.1]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%------
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.034]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
r_H =  [0.045];  %% [0.066 -0.011] respiration rate at 10° [gC/gN d ]
...
if OPT_SoilBiogeochemistry == 1
    %%%%%
    Wm_H= [1/19215]; %% 52.61 yr //  ; % wood turnover coefficient [1/d]
    B_H(1,1,6)= 12300; %% 7350; %%
    
    FNC_H(1,:)=[1];
    NupI_H(1,:)= [  0.0240479   0.000996224  0.01508];
    RexmyI(1,:)= [  0.02014170  0.1005269                 0];
    Nreserve_L(1,:)= [0]; Preserve_L(1,:)= [0]; Kreserve_L(1,:)= [0]; FNC_L(1,:)=[1];
    NavlI(1,:)=[  0.2226738   0.005789  0.5104997];
    %%%%
    %B_H(1,1,:) = [0 998 282 709 6 0 11 0];
    Nreserve_H(1,:)= 5; %
    Preserve_H(1,:)= 1.0;%
    Kreserve_H(1,:)= 0; %
    %%%%%%%

   P(1,:)=  1000*[   0.126448363136123   0.148427780651790   0.024034135410648   2.122265219223726   0.707418369432968   0.032416695395325,...
   0.243659950091450   0.063488862599685   2.704232515322099   0.823005632262451   6.256261900700758   0.014983148688075,...
   0.011069463704806   0.000063280459357   0.000041646370484   0.000026366858065   0.000069410617473   0.027383234941621,...
   0.098230870178384   0.045722211266678                   0   0.001604240608482   0.005870718109579   0.014653734833816,...
   0.003106192328068   0.727578831830986   0.005270008553969   0.015124390381137   0.002542228486749                   0,...
   0.000172999693371   0.000014963645459   0.000009555060901   0.000042224060848   0.000275029435306   0.001046696789690,...
   0.000185689405263   0.129388864248960   0.001708976634139   0.002458205242768   0.000381334273012                   0,...
   0.000002762891768   0.150000000000000   0.000297879327649   0.015000632936812   0.000000102342037   0.004551513177148,...
   0.007326867416907   0.000564158539311   0.011032308771468   0.000090422270388   0.000087961966209   0.003634154555070,...
   0.502967481553731];
...
Ws_under(1)=1;
%%%%%%%%%%%%%% Volume [mm]
O(1,:)= [0.117 0.117 0.118 0.018 0.117 0.114 0.195 0.194 0.194 0.195 0.197 0.198];
%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
%%%%%%%%%%%%%%%%%
cd(cur_dir)
