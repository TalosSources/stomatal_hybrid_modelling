...
%%% Dangxiong // The Damxung flux observation 
%%%% It represents a vegetation of dominate by Kobresia-Steppe in the transition zone of alpine meadow and alpine steppe.
%%% The vegetation of Kobresia-steppe meadow is dominated by Kobresia pygmaea C. B., Stipa capillacea Keng and Carex montis-everestii Kükenth dominate 
%the meadows, accompanied by other tussock sedges, grasses and herbs, all of which provide about 50-80% vegetation cover depending on annual precipitation. 
%The soil at the site is a sandy loam and classified as meadow soil. Its depth
%is 0.3–0.5 m, with high gravel content of 30%, organic matter of 0.9%–2.97%, total nitrogen
%of 0.05%–0.19%, total phosphorus of 0.03%–0.07%, and pH of 6.2–7.7
%% Sandy loam 
%% Canopy Height 0.15m 
%%  tower  height 2 m 
%% Growing season LAI = 1.88
%% GPP=330 
%%%%%%%%%%%%%%%%%
cur_dir=cd;
cd(Directory)
...
clear Oss_Lp Owp_Lp 
%%%%%%%%%%%%%
Zs = [ 0 10 20 50 100 150 200 250 300 400 500 ]; %%% [ms+1]
Zdes = 10;
Zinf=  10;
Zbio = 250; 
...
KnitL= [0.25]; %%% Canopy Nitrogen Decay
...
%%% Leaf
PsiL00_H =  [-1.3 ] ;%%[MPa]  Water Potential at 50% loss conductivity
PsiL50_H =  [-4.5 ]; %% [MPa]  Water Potential at 2% loss conductivity
Kleaf_max_H = [ 20 ] ; %%  %%%  [mmolH20 m^2 leaf s /MPa]
Cl_H  = [1200 ];  %%%  [500 - 3000]%  Leaf capacitance [mmolH20 / m^2 leaf MPa]
%%% Xylem
Axyl_H = [6.0] ; %% [cm^2 stem /m^2 PFT]
Kx_max_H = [80000];  %%5550-555550 [mmolH20 /m s MPa]  Xylem Conductivity specific for water;
PsiX50_H = [-6.5 ]; %%[MPa]  Water Potential at 50% loss conductivity
...
%%% Leaf
PsiL00_L =  [-1.1]; %% [MPa]  Water Potential at 2% loss conductivity
PsiL50_L =  [-3.5] ;%%[MPa]  Water Potential at 50% loss conductivity
Kleaf_max_L = [ 8] ; %%  %%%  [mmolH20 m^2 leaf s /MPa]
Cl_L  = [1200];  %%%  [500 - 3000]%  Leaf capacitance [mmolH20 / m^2 leaf MPa]
%%% Xylem
Axyl_L = [6.0] ; %% [cm^2 stem /m^2 PFT]
Kx_max_L = [80000];  %%5550-555550 [mmolH20 /m s MPa]  Xylem Conductivity specific for water;
PsiX50_L = [-9.5]; %%[MPa]  Water Potential at 50% loss conductivity
...
dc_C_H =  [2/365 ]; %% [1/ C] -- [Factor of increasing mortality]
...
Tlo_H = [12.2 ]; %% Mean Temperature for Leaf onset
...
LtR_H = [0.35]; %%% Leaf to Root ratio maximum
Mf_H= [0 ]; %% fruit maturation turnover [1/d]
...
Nl_L= [25 ]; %[gC/gN ] Leaf Carbon-Nitrogen ratio
[Stoich_L(1)]=Veg_Stoichiometric_Parameter(Nl_L(1));
%PLNR_L = 0.033; %% Percentage of Leaf N in Rubisco  [kgNRubisco / kgN]
r_L = [0.038 ];  %% [0.066 -0.011]respiration rate at 10?[gC/gN d ]
gR_L= [0.25 ]; % [0.22 - 0.28] growth respiration  [] -- [Rg/(GPP-Rm)]
%LAI_max_L= [2.0 10.0];
aSE_L= [2]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops
dd_max_L= [1/22 ];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_L = [7/365 ]; %% [1/ dï¿½C] -- [Factor of increasing mortality]
Tcold_L = [1.0 ]; %% [ï¿½C] Cold Leaf SLed
drn_L=  [1/950 ]; %% turnover root  [1/d]
...
Tlo_L = [2.5 ]; %% Mean Temperature for Leaf onset
Tls_L = [NaN ]; %% Mean Temperature for Leaf Shed
PAR_th_L= [NaN ]; 
dmg_L= [20]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L = [0.05 ];
Trr_L = [2.5]; %% Translocation rate [gC /m^2 d]
mjDay_L = [250]; %% Maximum Julian day for leaf onset
LDay_min_L =[13.1 ]; %% Minimum Day duration for leaf onset
LtR_L = [0.40 ]; %%% Leaf to Root ratio maximum
Mf_L= [1/60]; %% fruit maturation turnover [1/d]
Wm_L= [0 ] ; % wood turnover coefficient [1/d]
eps_ac_L = [0.5]; %% Allocation to reserve parameter [0-1]
LDay_cr_L = [12.5 ]; %%%  Threshold for senescence day light [h] [12.4 12.4]
Klf_L =  [1/25 ] ; % [1/83]; %% Dead Leaves fall turnover [1/d]
...
%Mpar_L(1).jDay_cut=[160:240];
%Mpar_L(1).LAI_cut=[-0.8]; %% Grazing %%
%%%%%%%%%%%% PRODUCTIVITY
Vmax_H = [0];
Vmax_L = [46];
%%%%%%%%%%%%%%%%%%%%%%
...
Ts(1)=Ta(1);
Tdamp(1)=0;
Tdp(1,:)= [  -10.4332  -10.2735   -9.7407   -8.8709   -8.1486   -7.8098   -7.8066   -8.0067   -8.4473   -8.9019];
TdpI_H(1,:)=0;
TdpI_L(1,:)=-7.5;
...
Ws_under(1,:)=1;
%%%%%%%%%%%%%% Volume [mm]
O(1,:)=  [  0.0331    0.0331    0.0331    0.0337    0.0343    0.0344    0.0341    0.0335    0.0331    0.0331];
Oice(1,:)=   [  0.0158    0.0164    0.0196    0.0670    0.0005    0.0372    0.0811    0.0979    0.1019    0.1069  ]; 
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%Vmin= 0*dz;
%Vmax= (Osat-Ohy)*dz;
