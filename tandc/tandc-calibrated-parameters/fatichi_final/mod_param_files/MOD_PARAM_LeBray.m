...
%%%%%%%%%%%%%%%%%%%% LE BRAY %%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%20 km SW of Bordeaux, colocated station at Belos (CarboAge). Experienced the 1999 storm and lost a large amount of trees, 
%Mediterranean/montane, evergreen coniferous plantation%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% SOIL AND HYDROLOGICAL PARAMETER
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% VEG. SPECIES 
%%% Upstore	--> maritime pine stand(P. Pinaster Ait.) planted in 1970.
%%% Understore --> The understorey consists mostly of grass (mainly
% Molinia coerulea L. Moench). The clumps of grass
% are about 0.7m high in early summer: they partly
% remain in winter, depending on frost. Berbigier et al 2001

%%LAI 3 and GPP 1200-1300 according to Granier et al 2007
%LAI 1.7 and GPP 1460 according to Peltoniemi et al 2012
%LAI 1.8-4.2 and sandy soil coarse according to van der Hoof et al 2013

%van der Hoof et al 2013: 12.5°C and 930mm
%Granier et al 13.5 950
%our 828mm 13.2°C


%%% LAI 3 but from understory too  2.6 in late winter and 3.1 in early autumn.
%%% GPP 1200  (1100-1300) 
%%% Root up to 80cm do not penetrate deeper layers 
%%%%%%%%%%%%%%%%%
cur_dir=cd;
cd(Directory)
...
Cbare = 0; Ccrown = [1.0];
%%%%%%%%%% SOIL INPUT %%% Sandy soil (similar to San Rossore
Pcla= 0.10; 
Psan= 0.80;
Porg= 0.015; 
Color_Class = 0;  
...
Oice = 0;
%%%
Zs= [0 10 50 100 200 300 400 500 600 700 800 1000 1200]; %%% [ms+1] Granier at al 2007
Zdes = 10;
Zinf=  10; 
Zbio = 250; 
...
ZR95_H = [800]; %% [mm] see Berbigier et al 2001... 800?
ZR95_L = [0]; %% [mm]
ZR50_H = [0];
ZR50_L = [0]; 
ZRmax_H = NaN;
...
rjv_H=[1.8]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ] 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Psi_sto_50_H =  -2.4 ;%% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_H =  -1.2; %% [MPa]  Water Potential at 2% loss conductivity
PsiL50_H =  -4.0 ;%%[MPa]  Water Potential at 50% loss conductivity
...
PsiX50_H = -8.0; %%[MPa]  Water Potential at 50% loss conductivity
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.009]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [45]; %[gC/gN ] Leaf Carbon-Nitrogen ratio 
...
dd_max_H= [1/350]; %%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  78/365; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_H = [-15]; %% [°C] Cold Leaf Shed
drn_H=  [1/930]; %% turnover root  [1/d]
dsn_H= [1/600]; % normal transfer rate sapwood [1/d] 
age_cr_H= [700]; %% [day] Critical Leaf Age
Bfac_lo_H= [0.95]; %% Leaf Onset Water Stress
Bfac_ls_H= [NaN]; %% Leaf Shed Water Stress [0-1]
Tlo_H = [11.0]; %% Mean Temperature for Leaf onset
Tls_H = [NaN NaN]; %% Mean Temperature for Leaf Shed
PAR_th_H= [NaN]; 
dmg_H= [30]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_H = [0.001];
Trr_H = [0.2]; %% Translocation rate [gC /m^2 d]
mjDay_H = [180]; %% Maximum Julian day for leaf onset
LDay_min_H =[12.1]; %% Minimum Day duration for leaf onset
LtR_H = [0.65]; %%% Leaf to Root ratio maximum 
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [0.015]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_L= [32]; %[gC/gN ] Leaf Carbon-Nitrogen ratio 
...
LDay_cr_L = [12.75]; %%%  Threshold for senescence day light [h]
...
ff_r_L= [NaN];
[ParEx_H(1)]=Exudation_Parameter(NaN);
...
%%%%%%%%%%%%%%%%
LAI_H(1,:)=[2.7]; %
B_H(1,:,:)= [277 422 426 300 5 0 15 0]; %%
Rrootl_H(1,:)= [5700] ;
PHE_S_H(1,:)=[1];
dflo_H(1,:)=[0];
AgeL_H(1,:)=[700];
e_rel_H(1,:)=[1];
hc_H(1,:) =[20.8]; %Granier et al, but Berbigier et al 2001 report 18 
SAI_H(1,:) = [0.1]; %% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
LAI_L(1,:)=[0.0];
...
%%%%%%%%%%%%%% Volume [mm]
%%%%%%%%%%%%%%%%%%%%%
O(1,:)=  [0.3787    0.3818    0.3876    0.3971    0.4098    0.4225    0.4352    0.4480    0.4607    0.4734    0.4925    0.4964]; 
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
