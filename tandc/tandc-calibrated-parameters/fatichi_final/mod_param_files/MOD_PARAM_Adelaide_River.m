...
%% Eucalyptos & Planchonia careya  ;; Sorghum 
% Clay = 20 Silt # Sand 50  -- Ks = 7 mm/h (A horizon: 0.3 m)
% Clay = 55 Silt # Sand 20  -- Ks = 5 mm/h (B horizon: 0.6 m)
% Clay = 9 Silt # Sand 26 
% Tree height 7.0 - 12.5
% GPP = 1880 gC/m2 
% LAI=1.1 (Total 0.75, Overstory 0.75)
% Ta = 26  Pr =1470  ET = 1120
%%% tower height 15 m 
% Soil moisture data: 5 cm (Howard Springs: 10cm)
% Open-forest/woodland savanna: typically 25% or 15-50% [Scholes & Archer,1997, Ann Rev Ecol Syst] (0.2 at Stuart plain)
% Low level grazing pressure, fire frequency 1 in 3 years
% mean biomass total: 100 g/m2
% mean litter biomass: 98 g/m2
%%
...
%%%%%%%%%%%%%%%%
LAI_H(1,:)=[2.69 0.0]; %
B_H(1,:,:)= [304 461 507 309 35 0 76 0 ;0 0 0 0 0 0 0 0]; %%
Rrootl_H(1,:)= [5408 0] ;
PHE_S_H(1,:)=[3 0];
dflo_H(1,:)=[115 0];
AgeL_H(1,:)=[226 0];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
LAI_L(1,:)=[0.0 0.66];
B_L(1,:,:)= [0 0 0 0 0 0 0 0 ; 66 0 165 118 10 0 2 0 ];
Rrootl_L(1,:)= [0 2087] ;
PHE_S_L(1,:)=[0 3];
dflo_L(1,:)=[0 72 ];
AgeL_L(1,:)=[0 41];
...
%%%%%%%%%%
%%%%
PARI_H(1,:,:)= [129.86 129.62 0.7377 ; 0 0 0];
NBLI_H(1,:)=  [ 1.697 0];
...
%%%%%%%%%%%%%% Volume [mm]
%%%%%%%%%%%%%%%%%%%%%
O(1,:)= [ 0.3123    0.3126    0.3128    0.3125    0.3118    0.3099    0.3054    0.2986    0.2890    0.2740    0.2389    0.2095    0.1664    0.1071, ... 
    0.1048    0.1324    0.1415    0.1485    0.1544    0.1616    0.1753    0.1990    0.2199    0.2309    0.2395    0.2403    0.2523]; 
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
