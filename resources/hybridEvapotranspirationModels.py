...
class Hybrid(pl.LightningModule):
    ...
    def shared_step(self,i): #shared step for train-val-test
        ...
        time_idx=i["time_idx"] 
        site_idx=i["site_idx"]
        Rs_hat,ra_hat,LE_hat = self(i)
        return Rs_hat,ra_hat,LE_hat,time_idx,site_idx
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int)  -> torch.Tensor: 
        ...
        Rs_hat,ra_hat,LE_hat,time_idx,site_idx=self.shared_step(batch)
        ...
        return {"loss": train_loss,'Rs_hat':Rs_hat.detach(), 'ra_hat':ra_hat.detach(),'LE_hat': LE_hat.detach(),'time_idx':time_idx.detach(),'site_idx':site_idx.detach()}

    def training_epoch_end(self, training_step_outputs) -> None:

        for var in ["LE_CORR_o","rs","ra"]:    
            var_new = var +  '_pred'    
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {'Rs_hat':Rs_hat, 'ra_hat':ra_hat,'LE_hat': LE_hat,'time_idx':time_idx,'site_idx':site_idx}
