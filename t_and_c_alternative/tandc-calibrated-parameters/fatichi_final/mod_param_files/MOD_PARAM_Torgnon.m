...
% Unmanaged grassland. 
% LAI increased from May and reached its annual maximum in mid-July. The maximum LAI in 2009 was 2.7 m2m?2 (DOY 194), 
% slightly lower than the 3 m2m?2in 2010 (DOY 201). LAI decreased earlier andsteeper in 2010 than in autumn 2009. (Rossini et al 2012 BG)
% EC Instruments were placed 2.5 m above the ground. (Calvagno et al 2013)
% The dominant vegetation is composed by Nardus stricta L., 35 %, Festuca nigrescens All. 11 %, Arnica montana L. 8 %, 
%Carex sempervirens Vill. 5 %, Geum montanum L. 5 %, Anthoxanthum alpinum L.L. 4 %, Potentilla aurea L. 4 %, Trifolium alpinum L. 4 %.
%Considering canopy properties, maximum sea-sonal LAI is about 2.9 and average canopy height is 18 cm. The terrain slopes gently (4 °) 
%and the soil is classified as Cambisol (WRB classification), with the following characteristics in the 0–20 cm horizon: organic C = 2.8 kg C m?2
%and total N = 0.22 kg N m?2. (Chapter 2 in The Greenhouse Gas Balance of Italy).

%%% Soil loamy sand (Sand 45-50 Clay 10-15)  pH 4.5 
...
Oice = 0;
%%%%%%%%%%%%%
Zs = [ 0 10 20 50 100 150 200 250 300 400 500 600 700 800 1000];  %%% [ms+1]
Zdes = 10;
Zinf=  10;
Zbio = 250;
...
DSE_L =0.650 ;  %% [kJ/mol] Activation Energy - Plant Dependent
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
Sl_L = [0.021]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_L= [19]; %[gC/gN ] Leaf Carbon-Nitrogen ratio
...
LDay_min_L =[12.2]; %% Minimum Day duration for leaf onset
LtR_L = [0.50]; %%% Leaf to Root ratio maximum
...
LDay_cr_L = [14.2]; %%%  Threshold for senescence day light [h]
Klf_L = [1/30]; %% Dead Leaves fall turnover [1/d]
...
%%%% Torgnon - Unmanaged 
%Mpar_L(1).jDay_cut=[240];
%Mpar_L(1).LAI_cut=[1.2]; %% LAI of grass after cut %% 2.65 Height of 15cm more or less
%%%%%%%%%%%%% PRODUCTIVITY
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
LAI_L(1,:)=[0.2]; %
B_L(1,:,:)= [0.91  0  298 236 0.9 0 141.6    0]; %%  
Rrootl_L(1,:) = [3396] ;
PHE_S_L(1,:)=[1];
dflo_L(1,:)=[0];
AgeL_L(1,:)=[176.8];
...
Ws_under(1)=1;
%%%%%%%%%%%%%% Volume [mm]
O(1,:)=  [ 0.3356    0.3358    0.3362    0.3371    0.3381    0.3390    0.3399    0.3434    0.3479    0.3520    0.3550    0.3573    0.3590    0.3609];
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%Vmin= 0*dz;
%Vmax= (Osat-Ohy)*dz;
