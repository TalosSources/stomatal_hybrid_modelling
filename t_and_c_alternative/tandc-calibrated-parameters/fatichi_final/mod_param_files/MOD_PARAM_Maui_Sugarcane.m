...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PLANT Saccharum officinarum L.
%% OBS July 23, 2011  - April 19, 2013

%%% Data 01.01. 2011 - 31.12 2013 
%%% Planting May 11, 2011 -- 
%$% Begin of mid-period (cover>80 %)  5 December 2011
%Mean  Rsw was 240.7 W/m2 Mean Ws = 4.1 m/s  Mean VPD 1130 
%%% During OBS July 23, 2011  - April 19, 2013
%%% Cumulative Re 3820 gC/m2 Cumulative GPP 9210 Cumulative NEE = 5371 
%%% Cumulative Aboveground biomass - 12500 gDM/m2 
%%% daily gross primary productivity (GPP) ranged from 1.9 to 25.9 gC/m2 
%observed precipitation at both adjacent weather stations was less than 160 mm for the entire 18–24 month period  Total rain 140 mm
%%% Normal Pr than 350 mm/year
%%% Irrigation 0.58 mm/h ; 4.62 mm/day  Total Irrigation 1928 mm 
%%% ET-EddyCov = 1389   1.6 to 5.5mm day-1 with a mean 3.8mm day-1 in Windy
%%%  ET PT sgane = 1600 - 1700 mm

%%% dry weight <-- fresh weigth  0.25-0.35 gDW/gFW 

%%% 20% fractional cover at the beginning -  LAI was 4.9+-0.2 

%%% Flux-tower observation period 
%%--> x1=4885; x2=20144; 
%%%--> x1=4885; x2 = 14515; %% for ET study 

%3.0–3.3 m above the zero-plane displacement (ZPD). ZPD was set as 2/3 of plant canopy height 
%Mean canopy height reached 3.97m in Lee and 4.09m in Windy by the end of the study.

%%% Sandy clay loam
%Soil texture – sand (%)  51
%Soil texture – silt (%)  16
%Soil texture – clay (%)  33

%%% Observations during 2 year sugarcane growth cycle
%% Continuous, irrigated, sugarcane cultivation for >100 years. 
%Practice is to grow plant sugarcane for 2 years, drydown, burn leaves, harvest cane, and then till and replant very shortly after harvest. 
%Site differs from Sugarcane Lee/Sheltered and Sugarcane Middle in soil type and site meteorology.

%Prior to planting, the soil is tilled to a depth of approximately 60 cm and soil amendments, including lime, mill
%mud, and sand, are added to balance soil pH and to fertilize according to commercial practice
...
KnitL=0.20; %%% Canopy Nitrogen Decay
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Psi_sto_50_H =  -3.0 ;%% [MPa]  Water Potential at 50% loss conductivity
...
%%------------------------
%
Psi_sto_00_L = -0.6;%  %% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_L = -3.0;%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_L =  -1.5; %% [MPa]  Water Potential at PLCs% loss conductivity
...
soCrop_H = [NaN]; 
Sl_emecrop_H = [NaN]; 
MHcrop_H =[NaN]; %%[m] maximum crop height  
[ParEx_H(1)]=Exudation_Parameter(0); 
[Mpar_H(1)]=Vegetation_Management_Parameter;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [0.018]; % 0.016-0.020; %%  0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
r_L = [0.035];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
...
dd_max_L= [1/150];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
...
age_cr_L= [730]; %% [day] Critical Leaf Age
...
PAR_th_L= -Inf; 
dmg_L= [35]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L = [0.1];
Trr_L = [4.0]; %% Translocation rate [gC /m^2 d]
mjDay_L = [250]; %% Maximum Julian day for leaf onset
LDay_min_L =[12.58]; %% Minimum Day duration for leaf onset
LtR_L = [0.7]; %%% Leaf to Root ratio maximum
...
soCrop_L = [0.3]; 
Sl_emecrop_L = [0.015]; 
MHcrop_L =[3.0]; %%[m] maximum crop height  
[ParEx_L(1)]=Exudation_Parameter(0); 
[Mpar_L(1)]=Vegetation_Management_Parameter;
%%%%%%%%%%%%%%%
Mpar_L(1).Date_sowing = datenum(2011,5,8,10,0,0) ; 
Mpar_L(1).Date_harvesting = datenum(2013,12,25,10,0,0) ; %%% Fully Harvested
Mpar_L(1).Crop_B=[35 35]; %%% [gC/m2] from sowing 
Mpar_L(1).Crop_crown = 0.90; 
%%%%
...
%B_L(1,:,:)= [0 0 50 290 0 0 0 0]; %%
%B_L(1,:,:)= [291 0 356 890 0 0 7 0]; %%
Rrootl_L(1,:) = [4250] ;
PHE_S_L(1,:)=1; %[3];
dflo_L(1,:)=[0];
AgeL_L(1,:)=0 ;%[730];
...
Ws_under(1)=1;
%%%%%%%%%%%%%% Volume [mm]
O(1,:)= [    0.2996    0.2996    0.2986    0.2946    0.2865    0.2651    0.2231    0.1902    0.1726    0.2179    0.2506    0.2718,...
    0.2848    0.2936    0.3026    0.3104    0.3158    0.3198    0.3226    0.3243]; 
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%Vmin= 0*dz;
%Vmax= (Osat-Ohy)*dz;
