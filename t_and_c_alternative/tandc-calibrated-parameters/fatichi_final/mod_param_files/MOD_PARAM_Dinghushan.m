...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% 
%South Subtropical evergreen broadleaved forest Ta 12-28 Pr = 1900 
% Major dominant species in canopy layers include Schima superba, Cleistocalyx Operculatus,Ayzygium jambos, Castanopsis chinensis Pinus massoniania, Rhododendron Moulmainens

% 38 m high observation tower 
%Soil Lateritic red earth  
%Canopy heigth 17  AG-Biomass 15000 gC m-2 LAI= 6.5
%% ET 630 GPP 1450 
...
zatm = 38; %% Reference Height
%%%%%%%%%%%%%%%%%% savanna cerrado (Eve. / Dec.)
%%%% LAI from 0.7 to  3.0  ;; ET = 1220  ;; 
...
Oice = 0;
%%%%%%%%%%%%%%%%%%
Zs= [0 10 50 100 200 300 400 500 600 700 800 1000 1250 1500 1750 2000]; %%% [ms+1] 
Zdes = 10;
Zinf = 10; 
Zbio = 250; 
...
rjv_H= [2.0 2.0];%[1.5]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ] 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Pss_H = [1000]; Pwp_H = [2500]; %%% [kPa]
%Pss_L=50; Pwp_L=300; %%% [kPa]
Psi_sto_00_H = [ -1.5 -0.6]; %% [MPa]  Water Potential at 2% loss conductivity
Psi_sto_50_H = [-2.8 -1.2] ;%% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_H =  [-1.8 -0.8]; %% [MPa]  Water Potential at 2% loss conductivity
PsiL50_H =  [-4.0 -2.0] ;%%[MPa]  Water Potential at 50% loss conductivity
...
%%------------------------
%%% Stomata 
Psi_sto_00_L = [-0.8 -0.8];%  %% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_L = [-2.5 -2.5];%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_L =  [-1.0 -1.0]; %% [MPa]  Water Potential at PLCs% loss conductivity
...
PsiX50_L = [-7.5 -7.5]; %%[MPa]  Water Potential at 50% loss conductivity
...
%%%% Growth Parameters 
PsiG50_H= [-1.8 -0.6];  %%[MPa]
PsiG99_H= [-4.0 -1.2];  %%[MPa]
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H =[0.022 0.020]; % 0.0012 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [40 35]; %[gC/gN ] Leaf Carbon-Nitrogen ratio 
[Stoich_H(1)]=Veg_Stoichiometric_Parameter(Nl_H(1));
[Stoich_H(2)]=Veg_Stoichiometric_Parameter(Nl_H(2));
r_H = [0.045 0.050] ;  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
...
dd_max_H= [1/200 1/30]; %%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  [1/365 1/365]; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_H = [5 5]; %% [°C] Cold Leaf Shed
drn_H= [1/510 1/510]; %% turnover root  [1/d]
...
Bfac_ls_H= [NaN 0.15]; %% Leaf Shed Water Stress [0-1]
Tlo_H = [8.0 14.0]; %% Mean Temperature for Leaf onset
Tls_H = [NaN NaN]; %% Mean Temperature for Leaf Shed
PAR_th_H= [0.75 NaN]; 
dmg_H= [45 30]; %%%  Day of Max Growth
LAI_min_H = [0.001 0.05];
Trr_H = [0.5 3]; %% Translocation rate [gC /m^2 d]
mjDay_H = [370 367]; %% Maximum Julian day for leaf onset
LDay_min_H =[10.4 12.1 ]; %% Minimum Day duration for leaf onset
LtR_H = [0.8 1.0]; %%% Leaf to Root ratio maximum 
...
LDay_cr_H = [9.0 11.2]; %%%  Threshold for senescence day light [h]
Klf_H =[1/20 1/10]; %% Dead Leaves fall turnover [1/d]
...
%%%%%%%%%%%%%%%%
LAI_H(1,:)=[3.77    2.21]; %
B_H(1,:,:)= [176 297 211 203 16 0 12 0; 
   210 274 210 189 9 0 3 0]; %%
Rrootl_H(1,:)= [2204 2223] ;
PHE_S_H(1,:)=[3 3];
dflo_H(1,:)=[0 0];
AgeL_H(1,:)=[269 217];
e_rel_H(1,:)=[1 1];
...
%%%%
PARI_H(1,:,:)= [59.37 63.2  -2.89; 
    59.37 63.2 -2.89];
NBLI_H(1,:)=[ 0.356    0.304];
