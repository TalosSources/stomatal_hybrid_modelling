...
%%%%%%%%%%%%%%%%%%
%%%%%%% VEG. SPECIES  --- by beech (Fagus sylvatica, 65%), ash (Fraxinus
%%%%%%% excelsior, 25%), and maple (Acer pseudoplatanus and A. platanoides,
%%%%%%% 7%) LAI 4.8 
...
Oice = 0;
%%%%%%%%%%%%%
Zs = [ 0 10 20 50 100 150 200 300 400 500 600 800 1000 1200]; %%% [ms+1]
Zdes = 10;
Zinf=  10;
Zbio = 250; 
...
mSl_L = 0.001; % [m2 LAI/gC * m2 PFT / m2 LAI]  0.0 - 0.004  Brod. Dec. Tree 
%%------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%------
...
Tlo_H = [5.0]; %% Mean Temperature for Leaf onset
...
LtR_H = [0.50]; %%% Leaf to Root ratio maximum
...
LDay_min_L =[11.1]; %% Minimum Day duration for leaf onset
...
if OPT_SoilBiogeochemistry == 1
    Wm_H= [1/22039]; %% 60.34 yr //  ; % wood turnover coefficient [1/d]
    B_H(1,1,6)=16064; %%
    
    FNC_H(1,:)=[1 ];
    NupI_H(1,:)= [ 0.035704022456261   0.002601284168432   0.016371266791523];
    RexmyI(1,:)= [ 0.020438223658889   0.120939606231860                   0];
    Nreserve_L(1,:)= [0]; Preserve_L(1,:)= [0]; Kreserve_L(1,:)= [0]; FNC_L(1,:)=[1];
    NavlI(1,:)=[0.161640850112723   0.002601224153884   0.065365609608091]; 
    %%%%
    %B_H(1,:,:)= [ 0 944 450 674 8 10881 0.26 0];
    Nreserve_H(1,:)= [2.55];
    Preserve_H(1,:)= [0.23];
    Kreserve_H(1,:)= [2.91];
   
  P(1,:)=  1000*[    0.211764541688496   0.254064701710665   0.041738677366436   2.640008692942835   0.880002897647611   0.045994857750059,...
   0.330102567127815   0.082431261473053   3.170136975626279   1.267356519017201   8.796615497584012   0.009883855635662,...
   0.007585042806074   0.000064202028056   0.000040533978093   0.000026750845024   0.000067556630154   0.038231283233505,...
   0.126767247803842   0.043381949962181                   0   0.001706046689949   0.006273061729020   0.018228554273980,...
   0.003585549038764   0.936997790842908   0.007358447454010   0.019517855968798   0.002412095090778                   0,...
   0.000172008843551   0.000014576652594   0.000009917050954   0.000168604668995   0.000325790307192   0.001302045147765,...
   0.000201574282501   0.176424612039641   0.002386598599781   0.003172347995089   0.000361814263617                   0,...
   0.000004321274655   0.150000000000000   0.000368757867806   0.015000520300412   0.000000106706319   0.004274011103416,...
   0.009114320356273   0.000604870948349   0.012036146918864   0.000067152418731   0.000058894200696   0.002260822490224,...
   0.502955887982222];
