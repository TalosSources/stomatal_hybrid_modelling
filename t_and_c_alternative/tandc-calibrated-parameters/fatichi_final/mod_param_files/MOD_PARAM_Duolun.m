...
%Stipa krylovii, grass 
%Agropyron cristatum, grass
%Artemisia frigida, semi-evergreen 
%Cleistogenes squarrosa, grass 
%Leymus chinensis grass

%The soil is chestnut soil (FAO classification) with 30cm depth, containing 76.8% sand, 16.7% silt and 6.5% clay. 
%Topsoil organic carbon and total nitrogen ( 0–10 c m) are 15.8g kg-1 and 1.7g kg-1, respectively.
 %The eddy covariance tower was built in May 2005 at a height of 5 m. Wind(4m)

%Heigth 0.4 m  AG-Biomass 106.9 gC m-2   LAI 1.0
%%% GPP 323 gC m-2 yr-1 
%%% ET 360 ;; Growing Season 150-254
%% LE  0.6 of Rn 
...
Tcold_H = [NaN  NaN]; %% [°C] Cold Leaf Shed
...
Sl_L = [0.016   0.008]; %  [m^2 gC] specific leaf area of  biomass [m^2 /gC] 0.05 -0.005
...
dd_max_L= 1./[50  365];%%%  [1/d]  death maximum for drought
dc_C_L =  1./[52  182]; %%[Factor of increasing mortality for cold]
Tcold_L = [0  -25]; %% [°C] Cold Leaf Shed
drn_L=  1./[1200   900]; %% turnover root  [1/d]
dsn_L= 1./[365  600]; % normal transfer rate sapwood [1/d]
age_cr_L= [200  730]; %% [day] Critical Leaf Age
Trr_L = [1.0   0.5]; %% Translocation rate [gC /m^2 d]
...
Tlo_L = [2.5  7]; %% Mean Temperature for Leaf onset
Tls_L = [NaN  NaN]; %% Not-used 
PAR_th_L= [Inf Inf]; %% Light Phenology Threshold 
dmg_L= [15  15]; %%%  Day of Max Growth
LAI_min_L = [0.05       0.001];
mjDay_L = [366  366]; %% Maximum Julian day for leaf onset
LDay_min_L =[13  13]; %% Minimum Day duration for leaf onset
LDay_cr_L = [11.1  11.1]; %%%  Threshold for senescence day light [h]
...
%%  1 cattle/ha == 7.2 sheep/ha
% Mpar_L(1).jDay_cut=[150:260];
% Mpar_L(1).LAI_cut=[-0.1675]; %% [gC /m^2 d]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Initial Condtion
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
L_day=zeros(NNd,1);
...
%%%%%%%%%%%%%%%%%%%%%
LAI_L(1,:)=[0.0 0];
B_L(1,:,:)= [0 0 183 149 1 0 90 0; 
     267 374 550 419 1 0 16 0];
Rrootl_L(1,:) = [3734 1068] ; 
PHE_S_L(1,:)=[1 1];
dflo_L(1,:)=[0 0];
AgeL_L(1,:)=[0 700];
e_rel_L(1,:)=[1 1];
...
if OPT_SoilBiogeochemistry == 1
    %%%%%
    %%%%
    Nreserve_L(1,:)= [0.2 0.2];
    Preserve_L(1,:)= [0.2 0.2];
    Kreserve_L(1,:)= [0.0 0.0];
    NupI_L(1,:,:)= [0.006002 1.23e-004     0.00358660
        0.02011100526  7.41e-004  0.0073449];
    RexmyI(1,:)= [ 0.184334304287017   0.000203982631625      0];
    NavlI(1,:)=[  0.718379626425977   0.208663946392677   0.135539703040235];
    %%%%
     P(1,:)=      1000*[   0.010991924987266   0.056888359455225   0.009403415416946                   0                   0   0.044756592952582,...
   0.028661883709326   0.003184653745481   0.486209263558958   0.731748711290154   3.774651117990187   0.022749712861546,...
   0.018857595841861   0.000029220799894   0.000017538220534   0.000012175333289   0.000029230367557   0.007751789028565,...
   0.023614104048308   0.000012986001789                   0   0.000320935563274   0.000912193172298                   0,...
   0.000503148625440   0.336100717121748   0.001500092429621   0.003632035360462   0.000017219235218                   0,...
   0.000946227197616                   0   0.000001646407084   0.000032093556326   0.000053220884719                   0,...
   0.000026579534186   0.060996763968991   0.000484080548765   0.000590214794786   0.000002582885283                   0,...
   0.000224495931137   0.150000000000000   0.001787958302646   0.015014887342300   0.000000016777205   0.000568515062982,...
                   0   0.000076804628297   0.003393869724844   0.000124654008799   0.000130238119258   0.000670761089314,...
   0.503241473224662];
     %%
    %%%%
...
Ts(1)=Ta(1)+1;
Tdamp(1)=Ta(1);
Tdp(1,:)= [   -23.5929  -23.4096  -23.0013  -22.2119  -21.3042  -20.5181  -19.5174  -18.5603  -17.8846  -17.4106  -16.8778  -16.3225  -15.8666  -15.6111];
TdpI_H(1,:)= -0.5242;
TdpI_L(1,:)= -0.5242;
...
Ws_under(1)=1;
%%%%%%%%%%%%%% Volume [mm]
O(1,:)= [  0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310    0.0310]; 
Oice(1,:)= [ 0.0688    0.0567    0.0392    0.0543    0.0809    0.1191    0.1416    0.1539    0.1135    0.1890    0.1169    0.1462    0.1394    0.1276]; 
%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
Vice(1,:) = Oice(1,:).*dz;
%%%%%%%%%%%%%%%%%
