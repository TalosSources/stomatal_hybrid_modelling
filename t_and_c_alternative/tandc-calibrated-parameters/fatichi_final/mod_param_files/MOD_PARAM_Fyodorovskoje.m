...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% RU-Fyo Fedorovskoje-drained spruce stand
%%% Fyodorovskoe in Tver, Russia  Ta 3.9 Pr 711 
%(Picea abies, 86%) and birch (Betula pubescens, 14%).
%The water table reaches the soil surface in spring, and during
%summer its depth is generally about 26 cm  19 cm.
%The soil root layer is 0–20 cm.
%%% Peatland 1m 

%LAI=3.5 GPP 102–122 mol CO2 m-2 yr-1 which is equal to 1200-1500
%(1998-2000) Milyukova et al 2002
%annual temperature and precipitation were checked and are close to reported
%values.
...
zatm = 44; %% Reference Height
OPT_WET=0; 
%%%%%%%%%%%%%%%%%%
%%%%%%% VEG. SPECIES  --- Site is an old (about 200 years) spruce forest with spruce  and birch. 
%The undergrowth is dominated by Vaccinium  and Sphaghum. The stand is approximately
% 27 m high, consisting of about 86% Picea abies trees
% and about 14% Betula spp. with a total live tree density
% of 565 ha?1 in 2000 (Vygodskaya et al., 2002) Aboveground
% woody biomass is approximately 63 tC ha?1
% (Schulze et al., 2002) with a projected leaf area index
% of about 3.5 (Schulze et al., 1999). The stand has a 60
% cm Sphagnum peat layer with the tree roots throughout the
% whole profile The soil C content in the peat layer is about
% 197 tonC ha?1 (Vygodskaya et al., 2002).
%%%% LAND COVER PARTITION
Cwat = 0.0; Curb = 0.0 ; Crock = 0.0;
Cbare = 0.0; Ccrown = [0.14 0.86];
%%%%%%%%%% SOIL INPUT peat clay see Schulze et al 1999
%Pcla= 0.73; 
%Psan= 0.05;
%Porg= 0.09; 
Pcla= 0.50; 
...
Oice = 0;
%%%%%%%%%%%%%%%%%%
Zs= [0 10 20 50 100 150 200 300 400 500 600 700 800 1000 1300 1500]; %% ms+1
Zdes = 10;
Zinf = 10; 
Zbio = 250; 
...
rjv_H= [2.2 2.1];%[1.5]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ] 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Psi_sto_50_H = [-2.0 -1.5] ;%% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
...
PsiX50_H = [-7 -7]; %%[MPa]  Water Potential at 50% loss conductivity
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H =[0.016 0.009]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [28 40]; %[gC/gN ] Leaf Carbon-Nitrogen ratio 
[Stoich_H(1)]=Veg_Stoichiometric_Parameter(Nl_H(1));
[Stoich_H(2)]=Veg_Stoichiometric_Parameter(Nl_H(2));
r_H = [0.035 0.055] ;  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
...
Tcold_H = [1.5 -40]; %% [°C] Cold Leaf Shed
drn_H= [1/1200 1/900]; %% turnover root  [1/d]
dsn_H= [1/1000 1/1100]; % normal transfer rate sapwood [1/d] 
age_cr_H= [140 950]; %% [day] Critical Leaf Age
Bfac_lo_H= [0.95 0.99]; %% Leaf Onset Water Stress
Bfac_ls_H= [NaN NaN]; %% Leaf Shed Water Stress [0-1]
PAR_th_H= [NaN NaN]; 
Tlo_H = [2.5 4.5]; %% Mean Temperature for Leaf onset
Tls_H = [NaN NaN]; %% Mean Temperature for Leaf Shed
dmg_H= [30 30]; %%%  Day of Max Growth
LAI_min_H = [0.01 0.001];
Trr_H = [4 0.25]; %% Translocation rate [gC /m^2 d]
mjDay_H = [250 220]; %% Maximum Julian day for leaf onset
LDay_min_H =[14.5 14.8]; %% Minimum Day duration for leaf onset
...
LDay_cr_H = [10.2 10.5]; %%%  Threshold for senescence day light [h]
...
%%%%%%%%%%%%%%%%
LAI_H(1,:)=[0 2.8]; %
B_H(1,:,:)= [0 421 276 302 9 0 0.4 0; 
   345 911 432 625 8 0 13 0]; %%
Rrootl_H(1,:)= [4200 3500] ;
PHE_S_H(1,:)=[3 3];
dflo_H(1,:)=[0 0];
AgeL_H(1,:)=[362 900];
e_rel_H(1,:)=[1 1];
...
Ts(1)=Ta(1)+2;
Tdamp(1)=3; 
Tdp(1,:)= [-0.4461   -0.4183   -0.3667   -0.2851   -0.2089   -0.1419   -0.0177    0.2388    0.5324    0.8082    1.0650    1.3005    1.6168    2.0081    2.1679]; 
TdpI_H(1,:)=3;
TdpI_L(1,:)=3;
