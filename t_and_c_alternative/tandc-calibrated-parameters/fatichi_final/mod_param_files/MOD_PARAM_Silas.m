...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% oak/pine-dominated plot consisting of 58% chestnut oak
%(Quercus prinus Willd), 14% black oak (Quercus velutina Lam), 6% scarlet oak (Quercus coccinia Münchh), 8%
%scattered pitch pine (Pinus rigida Mill), 6% white oak (Quercus alba L), and 3% post oak (Quercus stellata
%Wangenh) [see Schäfer et al., 2010)
%%% The canopy leaf area index (LAI) derived from litterfall was 1.7 in
%%% 2009. 
%%% 19m tower  MAT 11.5 MAP 1123
%The forest experienced defoliation by Gypsy moth (Lymantria dispar L.) in 2007, drought conditions in 2006 and a more severe
%drought in 2010
%%% Transpiration 201 mm/yr ±47
%%% ET 616 677 442 637 699 625 690
%%% sandy soils have a low water holding capacity,   0.865 (0.82-0.95) sand 0.055 (<0.02) clay
%%% field capacity  0.108 
%%% PH = 4.4 - 4.7 
%% Aboveground tree wood  prod. 60-90 gC/m2 yr 
%% Fine root biomass 48.
%% Aboveground tree wood  3800  gC/m2 
%%% maximum canopy height of 18.8
%% GPP 1100 ;; Litterfall 150 gC/m2 
%%% Soil depth -- 0.8 m 
...
zatm = 19; %% Reference Height
%%%%%%%%%%%%%%%%%%
%%%%%%% VEG. SPECIES  --- Pine / Oak 
%%%% LAND COVER PARTITION
Cwat = 0; Curb = 0 ; Crock = 0;
Cbare = 0; Ccrown = [0.08 0.92];
...
Oice = 0;
%%%%%%%%%%%%%
Zs= [0 10 20 50 100 150 200 300 400 500 600 700 800 1000 1200]; %% ms+1
Zdes = 10;
Zinf=  10;
Zbio = 250; 
...
Sp_LAI_H_In= [0.1 0.2]; %%[mm/LAI]
%%%%%%%%%%% Leaf Dimension
d_leaf_H= [0.25 4]; %%[cm]
d_leaf_L= [0 0];  %% [cm]
%%%%%%%% Biochemical parameter
%KnitH=0.35; %%% Canopy Nitrogen Decay
KnitH=[0.30 0.30]; %%% Canopy Nitrogen Decay
KnitL=[0.0 0.2] ; 
...
%Pwp_L = [2800 2800]; %%% [kPa]
%%% Stomata
Psi_sto_00_H =  [-0.8 -0.6]; %% [MPa]  Water Potential at 2% loss conductivity
Psi_sto_50_H =  [-2.0 -1.8] ;%% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_H =  [-1.0 -1.0] ;%%[MPa]  Water Potential at 50% loss conductivity
PsiL50_H =  [-2.6 -3.5]; %% [MPa]  Water Potential at 2% loss conductivity
...
%%%% Growth Parameters 
PsiG50_H= [-0.8;-0.6];  %%[MPa]
PsiG99_H= [-2.0; -1.8] ; %%[MPa]
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.011 0.022]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
%Sl_H = [0.0112 0];
Nl_H= [42 30]; %[gC/gN ] Leaf Carbon-Nitrogen ratio 
[Stoich_H(1)]=Veg_Stoichiometric_Parameter(Nl_H(1));
[Stoich_H(2)]=Veg_Stoichiometric_Parameter(Nl_H(2));
r_H = [0.055 0.050];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
gR_H= [0.25 0.25]; % [0.22 - 0.28] growth respiration  [] -- [Rg/(GPP-Rm)]
%LAI_max_H= [2.0 10.0];
aSE_H= [0 1]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops 
dd_max_H= [1/200 1/200]; %%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  [32/365 48/365]; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_H = [-20.0 6.0]; %% [°C] Cold Leaf Shed
drn_H=  [1/500 1/600]; %% turnover root  [1/d]
dsn_H= [1/450 1/730]; % normal transfer rate sapwood [1/d] 
age_cr_H= [500 140]; %% [day] Critical Leaf Age
Bfac_lo_H= [0.99 0.99]; %% Leaf Onset Water Stress
Bfac_ls_H= [NaN NaN]; %% Leaf Shed Water Stress [0-1]
Tlo_H = [9.5 6.5]; %% Mean Temperature for Leaf onset
Tls_H = [NaN NaN]; %% Mean Temperature for Leaf Shed
PAR_th_H= [NaN NaN]; 
dmg_H= [45 28]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_H = [0.001 0.05];
Trr_H = [0.7 6.0]; %% Translocation rate [gC /m^2 d]
mjDay_H = [250 250]; %% Maximum Julian day for leaf onset
LDay_min_H =[12.7 12.7]; %% Minimum Day duration for leaf onset
LtR_H = [0.9 0.9]; %%% Leaf to Root ratio maximum 
Mf_H= [1/50 1/50]; %% fruit maturation turnover [1/d]
Wm_H= [0 0] ; % wood turnover coefficient [1/d]
eps_ac_H = [0.25 1]; %% Allocation to reserve parameter [0-1] 
LDay_cr_H = [11.6 11.6]; %%%  Threshold for senescence day light [h]
Klf_H =[1/25 1/28]; %% Dead Leaves fall turnover [1/d]
fab_H = [0.74 0.75]; %% fraction above-ground sapwood and reserve
fbe_H = [0.26 0.25]; %% fraction below-ground sapwood and reserve
...
Tlo_L = [0 12.0]; %% Mean Temperature for Leaf onset 11.3 
...
LDay_min_L =[0 12.7]; %% Minimum Day duration for leaf onset 
...
LDay_cr_L = [0 11.7]; %%%  Threshold for senescence day light [h]
...
%%%%%%%
LAI_H(1,:)=[3.5 0]; %
B_H(1,:,:)= [302 479 336 327 3 0 15 0; 
     0 389 177 277 3 0 2 0  ]; %%
Rrootl_H(1,:)= [1000 1000] ;
PHE_S_H(1,:)=[4 4];
dflo_H(1,:)=[0 0];
AgeL_H(1,:)=[360 0];
e_rel_H(1,:)=[1 1];
