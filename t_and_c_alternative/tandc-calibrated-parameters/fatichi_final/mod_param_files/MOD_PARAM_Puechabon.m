...
%%%% Dense overstorey of the evergreen Holm oak (Q. ilex L.). Understorey coverage is low (<25%) and consists mainly of Buxus sempervirens, Phyllirea latifolia, Pistacia terebinthus and Juniperus oxycedrus
%%% Ta 13.5	 Pr 908
%%%% clear-cut in 1942
%% Soil silty clay loam soil (35% silt and 39% clay 26 sand ). A lot of stones  Depth 0.5
%%%%  About 90% of the root mass can be found in the first 50 cm, but some roots have been found up to 4.5 m deep
%%% GPP 1275 
%%% LAI = 2.8 
%%% ET 340 - 480 
%%% Tree height 5m  Tower height 10m
%%% AGB 12.3 gDM/m2 
...
zatm = 10; %% Reference Height
%%%%%%%%%%%%%%%%%%
%%%% LAND COVER PARTITION  
%%% Quercus Ilex  (LAI 3.2-3.8) 
...
Oice = 0;
%%%
Zs= [0 10 50 100 150 200 300 400 500 600 700 800 1000 1200 1500 2000 2500 3000 3500 4000 4500]; %%% [ms+1] 
Zdes = 10;
Zinf=  10; 
Zbio = 250; 
...
CASE_ROOT= 4 ;  %%% Type of Root Profile 
ZR95_H = [500]; %% [mm]
ZR95_L = [0]; %% [mm]
ZR50_H = [300];
ZR50_L = [0]; 
...
KnitH= 0.3; %%% Canopy Nitrogen Decay
KnitL= 0.5; 
...
%%% Stomata -1 -3.8 
Psi_sto_00_H =  -0.7; %% [MPa]  Water Potential at 2% loss conductivity
Psi_sto_50_H =  -3.5 ;%% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_H =  -1.4; %% [MPa]  Water Potential at 2% loss conductivity
PsiL50_H =  -5.5 ;%%[MPa]  Water Potential at 50% loss conductivity
...
Nl_H= [36]; %[gC/gN ] Leaf Carbon-Nitrogen ratio 
...
dd_max_H= [1/750]; %%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
...
age_cr_H= [650]; %% [day] Critical Leaf Age
Bfac_lo_H= [0.98]; %% Leaf Onset Water Stress
Bfac_ls_H= [NaN]; %% Leaf Shed Water Stress [0-1]
Tlo_H = [10.0]; %% Mean Temperature for Leaf onset
Tls_H = [NaN NaN]; %% Mean Temperature for Leaf Shed
PAR_th_H= [NaN]; 
dmg_H= [20]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_H = [0.001];
Trr_H = [1.0]; %% Translocation rate [gC /m^2 d]
mjDay_H = [180]; %% Maximum Julian day for leaf onset
LDay_min_H =[12.7]; %% Minimum Day duration for leaf onset
LtR_H = [0.9]; %%% Leaf to Root ratio maximum 
Mf_H= [1/80]; %% fruit maturation turnover [1/d]
Wm_H= [0] ; % wood turnover coefficient [1/d]
eps_ac_H = [0.4]; %% Allocation to reserve parameter [0-1] 
LDay_cr_H = [12.1]; %%%  Threshold for senescence day light [h]
...
Tlo_L =  [10.3]; %% Mean Temperature for Leaf onset
...
%%%%%%%%%%%%%%%%
LAI_H(1,:)=[2.8]; %
B_H(1,:,:)= [230 193 254 128 8 0 14 0]; %%
Rrootl_H(1,:)= [2657] ;
PHE_S_H(1,:)=[3];
dflo_H(1,:)=[0];
AgeL_H(1,:)=[635];
