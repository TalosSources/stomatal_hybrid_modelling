...
%dominant species at EOBS, while tall and aging jack pines (Pinus
%banksiana Lamb.) are scattered throughout the stand 
%fires that occurred between 1885 and 1915 (105- years old in 2008).   
%% Hc 14.1 m 
%% Tower height 24m 
%%% LAI 3.7  %%% 46% organic content 
%%% GPP 640 - 794 gC m-2 yr-1 
%%%% NPP = 300 
%%% Clay 4% Sand 50% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%
...
Oice = 0;
%%%%%%%%%%%%%
Zs= [0 10 20 50 100 150 200 300 400 500 600 700 800 1000 1500 2000 2500 3000 3500 4000 5000 ]; %% ms+1
Zdes = 10;
Zinf=  10;
Zbio = 250; 
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.015]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [42]; %[kgC/kgN ] Leaf Nitrogen Concentration
[Stoich_H(1)]=Veg_Stoichiometric_Parameter(Nl_H(1));
r_H = [0.065];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ] -- Bonan 2003
gR_H= [0.25]; % [0.22 - 0.28] growth respiration  [] -- [Rg/(GPP-Rm)]
aSE_H= [0]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Tropical Eq. forest --  
...
Tlo_H = [2.5]; %% Mean Temperature for Leaf onset 
...
Ts(1)=Ta(1)+2;
Tdamp(1)=Ta(1); 
Tdp(1,:)=   [ -0.3149   -0.3007   -0.2747   -0.2013   -0.0578   -0.0361   -0.0359   -0.0343   -0.0326   -0.0311   -0.0298,...
   -0.0286   -0.0269   -0.0262   -0.0263   -0.0264   -0.0262   -0.0259   -0.0255   -0.0248]; 
TdpI_H(1,:)=0;
TdpI_L(1,:)=0;
...
Ws_under(1)=1; 
%%%%%%%%%%%%%% Volume [mm]
O(1,:)= [     0.0902    0.0913    0.0933    0.0983    0.1107    0.1466    0.1533    0.1580    0.1618    0.1650    0.1677    0.1700,...
    0.1730    0.1786    0.1881    0.1911    0.1907    0.1904    0.1904    0.1963]; 
%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
