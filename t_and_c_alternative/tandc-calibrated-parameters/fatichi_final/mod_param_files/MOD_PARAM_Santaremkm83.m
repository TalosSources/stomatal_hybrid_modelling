...
Oice = 0;
%%%%%%%%%%%%%%%%%%
Zs= [0 10 50 100 200 300 400 600 800 1000 1250 1500 1750 2000 2500 3000 3500 4000 4500 5000 5500 6000 6500 7000 8000 9000 10000 12000 14000]; %%% [ms+1] 
Zdes = 10;
Zinf = 10; 
Zbio = 250; 
...
mSl_L = 0.0; % [m2 LAI/gC * m2 PFT / m2 LAI]  0.0 - 0.004  Brod. Dec. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
rjv_H= 2.2;%[1.5]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ] 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H =[0.019]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
%LAI_max_H= [2.0 10.0];
aSE_H= [3]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops 
...
age_cr_H= [270]; %% [day] Critical Leaf Age
...
LDay_min_H =[10.0]; %% Minimum Day duration for leaf onset
...
LDay_cr_H = [10.0]; %%%  Threshold for senescence day light [h]
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [0.012]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
%%%%%%%%%%%%%%%%
LAI_H(1,:)=[4.83]; %
B_H(1,:,:)= [266 490 212 338 27 0 25 0]; %%
Rrootl_H(1,:)= [2600] ;
PHE_S_H(1,:)=[3];
dflo_H(1,:)=[0];
AgeL_H(1,:)=[269];
...
%%%
PARI_H(1,:,:)= [ 85.0187  84.8007   -0.3993];
NBLI_H(1,:)= 1.6333;
PARI_L(1,:,:)=[0 0 0]; 
NBLI_L(1,:)=0;
%%%%
...
%%%%%%%%%%%%%%%%%%%%%
%ZWT(1)= 1999; %% [mm]
O(1,:)= [ 0.4156    0.4157    0.4157    0.4157    0.4158    0.4158    0.4159    0.4158    0.4157    0.4155    0.4151    0.4147    0.4143    0.4136, ... 
     0.4127    0.4120    0.4115    0.4112    0.4111    0.4111    0.4112    0.4114    0.4117    0.4121    0.4127    0.4133    0.4142    0.4148]; 
%dz(ms)=ZWT(1)-Zs(ms);
%%%%%%%%%%%%%%%%%%%%%%%%
%Vmin= 0*dz;
%Vmax= (Osat-Ohy)*dz;
