...
%%site info
%LAI=1.7 (3.6 all sided) 
% The stem density was 2100 ha?1 and the dominant tree height was
% 12.7 m. The trees were approximately 100 years old. The sparse
% ground vegetation consisted of lichens (73%), mosses (12%)
% and ericaceous shrubs (15%). The stand was located on fluvial
% sandy podzol. The soil texture was 0.3/91.2/4.3/4.2% of sand,
% till, clay and stones, respectively. The yearly average (1961–
% 1990) temperature and precipitation values were ?1.0 ?C and
% 499 mm, respectively. The ground is usually covered by snow
% from October until the middle of May, and the uppermost soil
% layers are frozen during the wintertime.

%Harkonen et al 2011 give time series 2004-2008 for:
%GPP: 550 551 588 437 438 
%NEE: 62 69 23 79 12
%temperature: 0.7 1.7 1.1 1.1 0.9 and
%precipitation: 602 843 615 738 596

%our average 548mm and 0.9
...
%%%%%%%%%%%%%%%%%%
%%%%%%% VEG. SPECIES  --- Pinus sylvestris
%%%% LAND COVER PARTITION
Cwat = 0; Curb = 0 ; Crock = 0;
Cbare = 0; Ccrown = [1.0];
%%%%%%%%%% SOIL INPUT -  0.3/91.2/4.3/4.2% of sand, till, clay and stones, respectively
% or Rautiainen_etal2012 70% sand, 29% silt
Pcla= 0.01;
Psan= 0.70;
Porg= 0.05;%0.05;
...
Oice = 0;
%%%%%%%%%%%%%
Zs= [0 10 20 50 100 200 300 400 500 700 900 1100 1500 ]; %% ms+1 see Rautiainen_etal2012, not explicitly mentioned
Zdes = 10;
Zinf=  10;
Zbio = 250; 
...
Ha_H =[75]; %% [kJ / mol K]  entropy factor - Plant Dependent 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% Matric Potential
%Pss_H = [850];  Pwp_H = [2800]; %%% [kPa] Pss_L = [1000];  Pwp_L = [4500]; %%% [kPa]
...
%%% HIGH VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.011]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_H= [38]; %[kgC/kgN ] Leaf Nitrogen Concentration
[Stoich_H(1)]=Veg_Stoichiometric_Parameter(Nl_H(1));
r_H = [0.060];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ] -- Bonan 2003
gR_H= [0.25]; % [0.22 - 0.28] growth respiration  [] -- [Rg/(GPP-Rm)]
aSE_H= [0]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops 
dd_max_H= [1/90];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_H =  78/365; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_H = [-40]; %% [°C] Cold Leaf Shed
...
mjDay_H = [240]; %% Maximum Julian day for leaf onset
LDay_min_H =[19.6]; %% Minimum Day duration for leaf onset
...
LDay_cr_H = [12.0]; %%%  Threshold for senescence day light [
...
%%%%%%%
LAI_H(1,:)=[1.6]; %
B_H(1,:,:)= [129 294 258 203 0.8 0 3 0]; %%
Rrootl_H(1,:)= [3000] ;
PHE_S_H(1,:)=[1];
dflo_H(1,:)=[0];
AgeL_H(1,:)=[920];
...
Ws_under(1)=1; 
%%%%%%%%%%%%%% Volume [mm]
O(1,:)= [0.2125    0.2128    0.2131    0.2132    0.2132   0.2132    0.2135    0.2151    0.2179    0.2212    0.2231    0.2260];
%%%%%%%%%%%%%%%%%%%
V(1,:) = (O(1,:) -Ohy).*dz;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
