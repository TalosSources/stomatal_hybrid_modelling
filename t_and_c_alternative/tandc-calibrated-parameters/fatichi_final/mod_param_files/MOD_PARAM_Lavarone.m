...
%IT-Lav: The Lavarone site is characterised by an uneven aged mixed forest dominated by Abies alba (70%), Fagus sylvatica (15%) and Picea abies (15%),
%with an average of 720 stems ha?1 (dbh > 12 cm) and a leaf area index (LAI) of 9.6, expressed as half of the total surface area, according to Chen and Black (1992).
%The canopy has a dominant layer reaching 33–36m and crown lower limit at about 12 m. In the understorey, suppressed fir and beeches form a discontinuous
%second layer from 0 to 4 m. The site is located on a gently rolling karst plateau and has a homogeneous vegetated fetch larger than 1 km in all directions,
%except for a 45? sector (300m fetch in the SSW direction). (Cescatti and Marcolla 2004).
%42-m-tall tower. The forest is dominated by silver fir (A. alba, 62.4% of basal area) and Norway spruce (P. abies, 35.6% of basal), 
%with a total basal area of 61.5 m2ha–1, a mean diameter at breast height (DBH) of 36 cm and a dominant height of 33 m.
%Because of the lack of management in previous decades, the stand has a high density (657 trees ha–1 with DBH > 17.5 cm), 
%a wide range in stem diameter (2.5 – 65 cm) and a multi-layered canopy. (Fiora and Cescatti 2006).

%Martinez et al 2016 Geoderma  GPP =2400; NPP =1568 ANPP =770; %% Pcla = 30 Psan = 0.22 
...
KnitH=[0.35 0.20]; %%% Canopy Nitrogen Decay
KnitL=[0.5 0.5]; 
...
DSE_H =[0.665 0.665];  %% [kJ/mol] Activation Energy - Plant Dependent
Ha_H =[65 94]; %% [kJ / mol K]  entropy factor - Plant Dependent
...
Psi_sto_50_H =   [-2.2 -2.5] ;%% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_H =  [-1.0 -1.0]; %% [MPa]  Water Potential at 2% loss conductivity
PsiL50_H =  [-3.0 -3.2] ;%%[MPa]  Water Potential at 50% loss conductivity
...
PsiX50_H = [-4.5 -5.5]; %%[MPa]  Water Potential at 50% loss conductivity
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_H = [0.022 0.012]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
r_H =  [0.028 0.058];  %% [0.066 -0.011] respiration rate at 10° [gC/gN d ]
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [0.032 0.032]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
Tlo_L = [1.0 1.0]; %% Mean Temperature for Leaf onset
Tls_L = [NaN NaN]; %% Mean Temperature for Leaf Shed
PAR_th_L= [NaN NaN]; 
dmg_L= [20 20]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L = [0.1 0.1];
Trr_L = [2.0 2.0]; %% Translocation rate [gC /m^2 d]
mjDay_L = [250 250]; %% Maximum Julian day for leaf onset
LDay_min_L =[10.7 10.7]; %% Minimum Day duration for leaf onset
...
%B_L(1,:,:)= [0 0 760 545 0 0 75 0;  0 0 0 0 0 0 0 0]; %% 
Rrootl_L(1,:)= [0 0] ;
PHE_S_L(1,:)=[1 1];
dflo_L(1,:)=[0 0];
AgeL_L(1,:)=[120 120];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
LAI_H(1,:)=[0 0];
B_H(1,:,:)= [ 0 333 293 299 8 0 60 0; 
    666 1590 606 1086 10 0 27 0];
Rrootl_H(1,:)= [4030 6200] ;
PHE_S_H(1,:)=[1 1];
dflo_H(1,:)=[0 0];
AgeL_H(1,:)=[0 1015];
e_rel_H(1,:)=[1 1];
