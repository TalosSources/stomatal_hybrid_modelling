...
%%% Maize Plantation Irrigated 
%A tillage operation (disking) was done just prior to the 2001 planting to homogenize the top 0.1 m of soil, 
% incorporate P and K fertilizers, as well as previously accumulated surface residues. Since the tillage operation, 
% the site has been under no-till management until the harvest of 2005. Following harvest, a conservation-plow tillage operation was initiated where a 
% small amount of N fertilizer is sprayed on the residue immediately prior to the plow operation. Approximately 1/3 of the crop residue is left on the surface. 
% The post-harvest conservation-plow operation continues as the current practice.

%a deep silty clay loam
%300-350 mm irrigation pivot from 25 June - 5 September 
%%% Planting 10 May - Harvesting 20 October 
%%% Tower height 6.0; %%  (3.0 m when the crop is less than 1 m)
%%% Yield 12.5 -14.0 Mg/ha --> 1250 g/m2 1400 g/m2  15-19% Moisture
%%% (1016 - 1140 g/m2)
%%% Content  (0.45 gC/gDM) 
%%% Peak LAI = 5.0-6.0  Biomass peak 750 gC/m2 
%%% GPP 1750 gC/m2 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%------
FI_L=[0.055];% Intrinsec quantum Efficiency [umolCO2/umolPhotons]
Do_L=[800]; %%[Pa]
a1_L=[4];
...
%%% Leaf
PsiL00_H =  -2.5 ;%%[MPa]  Water Potential at 50% loss conductivity
PsiL50_H =  -3.5; %% [MPa]  Water Potential at 2% loss conductivity
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [0.034]; % 0.016-0.020; %%  0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
...
PAR_th_L= [NaN]; %%% Stem allocation for Crops  
dmg_L= [45]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L = [0.1];
Trr_L = [4.5]; %% Translocation rate [gC /m^2 d]
mjDay_L = [200]; %% Maximum Julian day for leaf onset
LDay_min_L =[13.0]; %% Minimum Day duration for leaf onset
...
ff_r_L= [0.50];
soCrop_L = [0.05]; 
Sl_emecrop_L = [0.015]; %%% Additional SLA at emergence 
MHcrop_L =[2.5]; %%[m] maximum crop height  
[ParEx_L(1)]=Exudation_Parameter(0); 
[Mpar_L(1)]=Vegetation_Management_Parameter;
%%%%%%%%%%%%%%%
Mpar_L(1).Date_sowing = datenum(2001:1:2013,5,19,10,0,0) ; %% Date of Sowing
